...
for e in range(0, config.EPOCHS):
    ...
    for (triplet, _) in trainLoader:
        ...
        anchor_embeddings = bert_model.encode(anchor, convert_to_tensor=True)
        positive_embeddings = bert_model.encode(positive, convert_to_tensor=True)
        negative_embeddings = bert_model.encode(negative, convert_to_tensor=True)
